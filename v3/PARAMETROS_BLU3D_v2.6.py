...
class ParameterGraph:
    ...
    def plot_parameters(self, z_points, tool_points, feed_points, cool_points, act_drive_points):
        try:
            ...
            # Create x-ticks for Z heights
            z_ticks = np.arange(0, max(z_points) + 10, 10) if z_points else []

            # Define specific Z values to display
            specific_z_values = set(range(10, 101, 10))  # Z values from 10 to 100

            # Tool Speed line plot
            ...
            # Line Tool Speed values
            ax1.plot(z_points, tool_points, label='Tool Speed', color='#ffb3ff', marker='o')
            ax1.legend(loc='upper right')

            # Annotate specific Z values on Tool Speed plot
            for z in specific_z_values:
                if z in z_points:
                    tool_value = tool_points[z_points.index(z)]
                    ax1.text(z, tool_value, str(z), fontsize=8, ha='center', va='bottom')
            ...
            # Line Feed Rate values
            ax2.plot(z_points, feed_points, label='Feed Rate', color='#ffb3b3', marker='o')
            ax2.legend(loc='upper right')

            # Annotate specific Z values on Feed Rate plot
            for z in specific_z_values:
                if z in z_points:
                    feed_value = feed_points[z_points.index(z)]
                    ax2.text(z, feed_value, str(z), fontsize=8, ha='center', va='bottom')
            ...
            # Line Cooling values
            ax3.plot(z_points, cool_points, label='Cooling', color='#87CEEB', marker='o')
            ax3.legend(loc='upper right')

            # Annotate specific Z values on Cooling plot
            for z in specific_z_values:
                if z in z_points:
                    cool_value = cool_points[z_points.index(z)]
                    ax3.text(z, cool_value, str(z), fontsize=8, ha='center', va='bottom')
            ...
            # Line ACT_DRIVE values
            ax4.plot(z_points, act_drive_points, label='ACT_DRIVE', color='#90EE90', marker='o')
            ax4.legend(loc='upper right')

            # Annotate specific Z values on ACT_DRIVE plot
            for z in specific_z_values:
                if z in z_points:
                    act_drive_value = act_drive_points[z_points.index(z)]
                    ax4.text(z, act_drive_value, str(z), fontsize=8, ha='center', va='bottom')
    ...
    def extract_parameters(self):
        ...
        for line in self.original_content.splitlines():
            ...
            tool_match = re.search(r'TOOL_RPM\s*=\s*([\d.]+)', line)
            if tool_match:
                tool_points.append(float(tool_match.group(1)))

            feed_match = re.search(r'\$VEL\.CP\s*=\s*([\d.]+)', line)
            if feed_match:
    ...
    def update_graph_with_loaded_params(self):
        ...
        if not all(len(lst) == len(z_points) for lst in [tool_points, feed_points, cool_points, act_drive_points]):
        ...
        # Additional debug output to confirm values are being passed correctly
        print("Graph updated with:")
        print("Z Points:", z_points)
        print("Tool Points:", tool_points)
        print("Feed Points:", feed_points)
        print("Cool Points:", cool_points)
