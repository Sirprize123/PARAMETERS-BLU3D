...
class ParameterGraph:
    def __init__(self, master):
        ...
        self.ax.set_xlabel('Print Progress (%)')  # Changed X axis to Print Progress
        self.ax.grid(True)
        
        # Create second y-axis for cooling
        self.ax2 = self.ax.twinx()
        
        # Store lines for updating
        self.lines = {}
        
    def plot_parameters(self, z_points, tool_points, feed_points, cool_points, act_drive_points):  # {{ edit_1 }}
        try:
            # Clear previous plots
            self.figure.clear()
            
            # Create subplots
            ax1 = self.figure.add_subplot(411)  # Tool Speed
            ax2 = self.figure.add_subplot(412)  # Feed Rate
            ax3 = self.figure.add_subplot(413)  # Cooling
            ax4 = self.figure.add_subplot(414)  # ACT_DRIVE  # {{ edit_2 }}
            
            # Set labels and grid for each subplot
            ax1.set_xlabel('Z Height (mm)')
            ax1.set_ylabel('Tool Speed (rpm)')
            ax1.grid(True)
            ax1.set_ylim(0, max(tool_points) if tool_points else 0)  # Set Y-axis limit for Tool Speed
            
            ax2.set_xlabel('Z Height (mm)')
            ax2.set_ylabel('Feed Rate (mm/s)')
            ax2.grid(True)
            ax2.set_ylim(0, 2)  # Set Y-axis limit for Feed Rate (max = 2)
            
            ax3.set_xlabel('Z Height (mm)')
            ax3.set_ylabel('Cooling (%)')
            ax3.grid(True)
            ax3.set_ylim(0, 100)  # Set Y-axis limit for Cooling (max = 100)
            
            ax4.set_xlabel('Z Height (mm)')  # {{ edit_3 }}
            ax4.set_ylabel('ACT_DRIVE')  # {{ edit_4 }}
            ax4.grid(True)
            ax4.set_ylim(0, 1)  # Set Y-axis limit for ACT_DRIVE (assuming binary values)  # {{ edit_5 }}
            
            # Plot parameters with markers
            if tool_points:
            ...
            if act_drive_points:  # {{ edit_6 }}
                ax4.plot(z_points, act_drive_points, label='ACT_DRIVE', color='#90EE90', marker='o')  # {{ edit_7 }}
                ax4.legend(loc='upper right')  # {{ edit_8 }}
...
class SRCModifierApp:
    ...
    def load_file(self):
        try:
            ...
            if self.extract_params_from_file():
                ...
                self.update_graph_with_loaded_params()  # Ensure this line is present
    ...
    def update_graph_with_loaded_params(self):  # {{ edit_1 }}
        z_points = []
        tool_points = []
        feed_points = []
        cool_points = []
        act_drive_points = []  # {{ edit_2 }}

        # Extract parameter values from self.original_content
        for line in self.original_content.splitlines():
            ...
            elif 'ACT_DRIVE=' in line:  # {{ edit_3 }}
                act_drive_match = re.search(r'ACT_DRIVE\s*=\s*(TRUE|FALSE)', line)
                if act_drive_match:
                    act_drive_points.append(1.0 if act_drive_match.group(1) == 'TRUE' else 0.0)  # Convert to 1.0 or 0.0

        # Ensure all lists have the same length by filling missing values
        max_length = max(len(z_points), len(tool_points), len(feed_points), len(cool_points), len(act_drive_points))  # {{ edit_4 }}
        while len(z_points) < max_length:
            z_points.append(z_points[-1] if z_points else 0)
        while len(tool_points) < max_length:
            tool_points.append(tool_points[-1] if tool_points else 0)
        while len(feed_points) < max_length:
            feed_points.append(feed_points[-1] if feed_points else 0)
        while len(cool_points) < max_length:
            cool_points.append(cool_points[-1] if cool_points else 0)
        while len(act_drive_points) < max_length:  # {{ edit_5 }}
            act_drive_points.append(act_drive_points[-1] if act_drive_points else 0)

        # Update the graph with the extracted values
        self.parameter_graph.plot_parameters(z_points, tool_points, feed_points, cool_points, act_drive_points)  # {{ edit_6 }}
    ...
    def create_ui(self):
        try:
            ...
            self.parameter_graph = ParameterGraph(graph_frame)
    ...
    def update_preview(self):
        try:
            ...
            if z_points and (tool_points or feed_points or cool_points):
                ...
                self.parameter_graph.plot_parameters(z_points, tool_points, feed_points, cool_points)
