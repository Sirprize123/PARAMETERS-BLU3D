...
class ParameterGraph:
    ...
    def plot_parameters(self, z_points, tool_points, feed_points, cool_points):
        try:
            ...
            self.ax.set_ylabel('Tool Speed (rpm) / Feed Rate (mm/s)')
            self.ax2.set_ylabel('Cooling (%)')
            self.ax.grid(True)
            
            # Plot parameters
            if tool_points:
                self.lines['tool'] = self.ax.plot(z_points, tool_points, 'r-', label='Tool Speed', color='#ffb3ff')
            if feed_points:
                self.lines['feed'] = self.ax.plot(z_points, feed_points, 'b-', label='Feed Rate', color='#ffb3b3')
            if cool_points:
                self.lines['cool'] = self.ax2.plot(z_points, cool_points, 'g-', label='Cooling', color='#87CEEB')
