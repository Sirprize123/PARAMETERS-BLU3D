...
class SRCModifierApp:
    def __init__(self, root):
        try:
            ...
            # Add frame for file metadata editing
            self.metadata_frame = tk.LabelFrame(self.root, text="File Settings")
            self.metadata_frame.pack(fill='x', padx=2, pady=2)
            
            # DEF name entry
            self.def_frame = tk.Frame(self.metadata_frame)
            self.def_frame.pack(fill='x', padx=5, pady=2)
            tk.Label(self.def_frame, text="Display Name:").pack(side='left', padx=5)
            self.def_entry = tk.Entry(self.def_frame)
            self.def_entry.pack(side='left', fill='x', expand=True)
            
            # PARKPOS entry
            self.parkpos_frame = tk.Frame(self.metadata_frame)
            self.parkpos_frame.pack(fill='x', padx=5, pady=2)
            tk.Label(self.parkpos_frame, text="PARKPOS:").pack(side='left', padx=5)
            self.parkpos_entry = tk.Entry(self.parkpos_frame)
            self.parkpos_entry.pack(side='left', fill='x', expand=True)
            
            # Update button
            tk.Button(self.metadata_frame, text="Update Settings", 
                     command=self.update_file_settings).pack(pady=5)
    ...
    def update_file_settings(self):
        try:
            ...
            for i, line in enumerate(lines):
                if line.startswith("DEF "):  # Modify DEF line
                    lines[i] = f"DEF {self.def_entry.get()}"
                    modified = True
                elif line.startswith("PARKPOS = "):  # Modify PARKPOS line
                    lines[i] = f"PARKPOS = {self.parkpos_entry.get()}"
                    modified = True
            ...
            if lines:
                for line in lines:
                    if line.startswith("DEF "):
                        ...
                        parkpos_value = line.split("PARKPOS = ", 1)[1]
                        self.parkpos_entry.delete(0, tk.END)
                        self.parkpos_entry.insert(0, parkpos_value)
    ...
    def add_z_height(self):
        self.add_frame("Add Z Height", "Z Height")
            ...
            #     # Create dialog window
            #     dialog = tk.Toplevel(self.root)
            #     dialog.title("Add Z Height")
            #     dialog.geometry("300x150")  # Set reasonable size
            ...
            #             self.create_z_height_frame(z_height)
                        
            #             # Add the line to preview at the current cursor position
            #             if self.preview_text:
            #                 current_pos = self.preview_text.index(tk.INSERT)
                            
            #                 # Insert into preview
            #                 self.preview_text.insert(current_pos, f"LIN {{X __.__, Y __.__, Z {z_height:.2f}}} C_DIS\n")
                            
            #                 # Insert into original content at same position
            #                 line_num = int(current_pos.split('.')[0])
            #                 content = self.original_content.splitlines()
            #                 content.insert(line_num-1, f"LIN {{X __.__, Y __.__, Z {z_height:.2f}}} C_DIS")
            #                 self.original_content = '\n'.join(content) + '\n'
            ...
            #     # Add button
            #     tk.Button(dialog, text="Add", command=validate_and_add).pack(pady=5)
                
            # except Exception as e:
            #     messagebox.showerror("Error", f"Failed to add Z height: {str(e)}")
    ...
    #         # Prompt for Z height value
    #         z_height = simpledialog.askfloat("Add Z Height", "Enter Z height value:")
    #         if z_height is None:
    #             return

    #         # Validate Z height
    #         max_z = self.get_max_z_value()
    #         print(f"the max z is {max_z}")
    #         if z_height < 0 or z_height > max_z:
    #             messagebox.showerror("Error", f"Z height must be between 0 and {max_z}.")
    #             return

    #         # Find the line with matching Z height
    ...
    #                 match_z = abs(float(match.group(1)))
                
    #                 #assert match_z == 13.0
    #                 print(f"the match is {match_z}")
    #                 if match_z == 13.0:
    #                     print(f"the match is {abs(float(match.group(1)))}")
    #                     bp = 0
                
                
                
                
    #             if match and abs(float(match.group(1)) - z_height) < 0.0001:
    #                 print(f"DSGDFGSDFGDFS")
    #                 line_found = True
    #                 if z_height not in self.custom_z_params:
    #                     self.custom_z_params[z_height] = {}

    #                 # Create a frame for the new z height parameter
    #                 self.create_print_progress_frame(z_height, True)
    ...
    #                 # After adding z height, prompt for parameter
    #                 self.add_param_to_progress(z_height, True)
    #                 break

    #         if not line_found:
    #             messagebox.showerror("Error", f"No line found with Z height {z_height}")
        ...
        for line in self.original_content.splitlines():
            ...
            if match:
                ...
                if z_value > max_z:
                    max_z = z_value
        return max_z
    # def create_z_height_frame(self, value):
    #     # Create new frame with appropriate title  
    #     title = f"Z Height: {value}"
    #     z_frame = tk.LabelFrame(self.param_frame, text=title)
    #     z_frame.pack(fill='x', padx=5, pady=2)
    #     self.z_param_frames[value] = z_frame
        
    #     # Add plus button to add new parameters
    #     add_btn = tk.Button(z_frame, text="+", command=lambda: self.add_param_to_progress(value, True))
    #     add_btn.pack(side='right', padx=2)
        
    #     # Display existing parameters for this z height
    #     self.refresh_progress_params(value, True)
    ...
    def create_ui(self):
        try:
            ...
            # Load button
            self.load_button = tk.Button(left_frame, text="Load File", command=self.load_file)
            ...
            self.entries = {}
            # Output name entry
            output_label = tk.Label(left_frame, text="Output filename:")
            output_label.pack(pady=(5,0))
            self.output_name = tk.Entry(left_frame, width=30)
            self.output_name.pack(pady=5)
            
            # Get input file name without extension and set default output name
            input_name = os.path.splitext(os.path.basename(self.input_file))[0] if self.input_file else ""
            default_output = f"{input_name}_modified.src" if input_name else ""
            self.output_name.insert(0, default_output)

            # Modify button
            self.modify_button = tk.Button(left_frame, text="Modify & Save", command=self.modify_file, state=tk.DISABLED)
            ...
            find_next_button = tk.Button(search_frame, text="Find Next", command=find_text)
            find_next_button.pack(side='left')
    ...
    def update_line_and_preview(self, key, value_var, line_number):
        try:
            ...
            if "Tool Speed" in param_type:
                content[line_idx] = f"TOOL_RPM={value}"
            elif "Feed Rate" in param_type:
    ...
    def modify_file(self):
        try:
            ...
            # Create default output and changelog names
            default_output = f"{input_name}_modified.src"
            default_changelog = f"{input_name}_modified_changelog.txt"
            
            # Use default output name if none provided
            output_file = self.output_name.get() if self.output_name.get() else default_output
            if not output_file.endswith('.src'):
                output_file += '.src'
            ...
            # Append to changelog using default name
